...
if (isset($_POST['btnupdateorder'])) {
  ...
  $selectdetailsv = query("SELECT order_date from tbl_invoice where invoice_id =$id");
  $rowc = $selectdetailsv->fetch_object();
  
  $_SESSION['order_datedd'] = $rowc->order_date;
  $order_dated = $_SESSION['order_datedd'];
  ...
  $update_tbl_invoice = query("UPDATE tbl_invoice SET subtotal='{$txt_subtotal}',discount='{$txt_discount}',discountp='{$txt_discountp}',free='{$free}',discount_h='{$discount_h}',total='{$txt_total}',payment_type='{$txt_payment_type}',due='{$txt_due}',paid='{$txt_paid}',edit='{$saler_name}' where invoice_id=$id");
  ...
  if ($invoice_id != null) {
    ...
    for ($i = 0; $i < count($arr_pid); $i++) {
      ...
      $insert = query("INSERT into tbl_invoice_details (invoice_id,barcode,product_id,product_name,qty,original_price,rate,saleprice,order_date,saler_id,aus) values ('{$id}','{$arr_barcode[$i]}','{$arr_pid[$i]}','{$arr_name[$i]}','{$arr_qty[$i]}','{$purchaseprice_arr[$i]}','{$arr_price[$i]}','{$arr_total[$i]}','{$order_dated}','{$saler_id}','{$aus}')");
      confirm($insert);
    } //end for loop
