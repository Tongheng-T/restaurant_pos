<script>
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (jQuery.inArray(data["pid"], productarr) !== -1) {
                        ...
                        addrow(data["pid"], data["product"], data["saleprice"], data["stock"], data["barcode"], data["image"], data["purchaseprice"]);

                        productarr.push(data["pid"]);

                        //$("#txtbarcode_id").val("");

                        function addrow(pid, product, saleprice, stock, barcode, image, purchaseprice) {
    ...
    $("#itemtable").delegate(".qty", "keyup change", function() {
        ...
        if ((quantity.val() - 0) > (tr.find(".stock_c").val() - 0)) {
            ...
            // tr.find(".totalamt_original").text(quantity.val() * tr.find(".purchasepriceo").val());
    ...
    function calculate(dis, paid) {
        ...
        var discountt = $("#discount_h").val();
        var discount_free = $("#discount_free").val();
        ...
        var dd = '';
        total = subtotal - discount - discountt - discount_free;
        due = paid_amt - total;
