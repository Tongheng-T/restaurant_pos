    ...
    success: function(data) {
      ...
      $.each(data, function(key, data) {
        if (jQuery.inArray(data["product_id"], productarr) !== -1) {
          ...
          addrow(data["product_id"], data["product_name"], data["qty"], data["rate"], data["saleprice"], data["stock"], data["barcode"], data["image"], data["purchaseprice"]);

          productarr.push(data["product_id"]);

          //$("#txtbarcode_id").val("");

          function addrow(product_id, product_name, qty, rate, saleprice, stock, barcode, image, purchaseprice) {
            ...
            var tr = '<tr>' +
              ...
              '<td style="text-align:left; vertical-align:middle; font-size:17px;"><span class="badge badge-warning " ' + product_id + '">' + purchaseprice + '</span><input type="hidden" class="form-control price_c" name="purchaseprice_arr[]" id="purchaseprice' + product_id + '" value="' + purchaseprice + '"></td>' +
