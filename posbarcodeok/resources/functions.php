...
function esc($value)
{
    global $connection; // សន្មត់ $conn ជា mysqli connection object
        ...
        Swal.fire({
          ...
          title: "Max file should be 1MB"
                    ...
                    if (move_uploaded_file($f_tmp, $store)) {
                        ...
                        $update = query("UPDATE tbl_product set barcode='$barcode_txt', product='$product_txt' , category='$category_txt' , description='$description_txt' , stock='$stock_txt' , purchaseprice='$purchaseprice_txtt' , saleprice='$saleprice_txtt' , image='$f_newfile' where pid=$id");
                        confirm($update);
          ...
          title: "Product Update Failed"
            ...
            $update = query("UPDATE tbl_product set barcode='$barcode_txt', product='$product_txt' , category='$category_txt' , description='$description_txt' , stock='$stock_txt' , purchaseprice='$purchaseprice_txtt' , saleprice='$saleprice_txtt' , image='$image_db' where pid=$id");
            confirm($update);
            if ($update) {
...
function stock_list()
{
    ...
    while ($row = $select->fetch_object()) {
        ...
        if ($usd_or_real == "usd") {
            ...
            $total = number_format($row->price * $row->stock, 2) . $USD_usd;
